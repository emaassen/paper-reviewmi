...
df <- read_excel("../data/codebook-main-step1.xlsx")
df0f <- df[df$journal_id == 0,] # dataset to save
df1f <- df[df$journal_id == 1,] # dataset to save

n_art_df0 <- count_articles(df0f); n_art_df0        # 213 articles                                   # 342 articles
n_art_df1 <- count_articles(df1f); n_art_df1        

# Quantitative data
df0 <- filter(df0f, empirical == 1)
df1 <- filter(df1f, empirical == 1)
count_articles(df0)                             # 141 articles                             
count_articles(df1)                             # 201 articles

# Number of articles and studies that dropped out due to non-quantitative data
n_art_df0 - count_articles(df0)                  # 72 articles
n_art_df1 - count_articles(df1)                  # 12 articles

# Group comparison 
df00 <- filter(df0, compare_group == 1)
df11 <- filter(df1, compare_group == 1)
count_articles(df00)                             # 107 articles                             
count_articles(df11)                             # 167 articles

# Number of articles and studies that dropped out due to non-group comparisons
count_articles(df0) - count_articles(df00)       # 34 articles
count_articles(df1) - count_articles(df11)       # 34 articles

# Comparison made on scale 
df000 <- filter(df00, scale == 1)
df111 <- filter(df11, scale == 1)
count_articles(df000)                             # 70 articles                             
count_articles(df111)                             # 124 articles

# Number of articles and studies that dropped out due to non-scales
count_articles(df00) - count_articles(df000)       # 37 articles
count_articles(df11) - count_articles(df111)       # 43 articles

# Comparison made on reflective scale 
df0000 <- filter(df000, reflective == 1)
df1111 <- filter(df111, reflective == 1)
count_articles(df0000)                             # 51 articles                             
count_articles(df1111)                             # 46 articles
nrow(df0000)                                       # 642 comparisons
nrow(df1111)                                       # 276 comparisons

# Number of articles and studies that dropped out due to non reflective scales
count_articles(df000) - count_articles(df0000)       # 19 articles
count_articles(df111) - count_articles(df1111)       # 78 articles
...
# MI tested
sum(df0$mitest_rep)                  # 34 scales reported MI testing
count_articles(df0)                  # 51 articles remaining
sum(df1$mitest_rep)                  # 6 scales reported MI testing
count_articles(df1)                  # 46 articles remaining
table(df0$mitest_rep)                # 0 = 608, 1 = 34
table(df1$mitest_rep)                # 0 = 270, 1 = 6

# Only select rows that report on MI
dfmi0 <- subset(df0,mitest_rep == 1)
dfmi1 <- subset(df1,mitest_rep == 1)
N0 <- nrow(dfmi0);N0
N1 <- nrow(dfmi1);N1
count_articles(dfmi0) # 4
count_articles(dfmi1)  # 2

# MI result
df0$miresult_rep <- as.numeric(df0$miresult_rep)  
df1$miresult_rep <- as.numeric(df1$miresult_rep)  
table(df0$miresult_rep, useNA="always")  # 25 measurement invariance, 9 no measurement invariance
table(df1$miresult_rep, useNA="always")  # 6 measurement invariance, 0 no invariance measurement invariance

# Subset those that tested for MI 
dftest0 <- subset(df0,mitest_rep == 1)
dftest1 <- subset(df1,mitest_rep == 1)
table(dftest0$milevel_rep)   # 1 metric invariance, 2 scalar invariance, 1 partial invariance, 30 MIMIC
table(dftest1$milevel_rep)   # 5 metric invariance, 1 scalar invariance

# Subset those that did not find any MI and those that did find MI
dftest0y <- subset(dftest0,miresult_rep == 1)     
dftest0n <- subset(dftest0,miresult_rep == 0)    
dftest1y <- subset(dftest1,miresult_rep == 1)     
dftest1n <- subset(dftest1,miresult_rep == 0) 

table(dftest0y$milevel_rep)     # 1 metric invariance, 2 scalar invariance, 22 MIMIC method
table(dftest0n$milevel_rep)     # 1 partial invariance, 8 MIMIC

table(dftest1y$milevel_rep)     # 5 metric invariance, 1 scalar invariance
table(dftest1n$milevel_rep)     
