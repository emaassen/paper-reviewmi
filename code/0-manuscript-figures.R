...
type <- c("Between-group: demographic","Between-group: experimental","Within-group: experimental","Scale: existing","Scale: modified")[match (type, c("dem","exp_misc","exp_time","0","1"))]
...
tab <- within(tab,
              ...
              Type <- factor(Type,levels=c("Between-group: demographic","Between-group: experimental","Within-group: experimental","Scale: existing","Scale: modified")))
...
tab <- filter(tab, Type == "Between-group: demographic" | Type == "Between-group: experimental" | Type == "Within-group: experimental")
...
fig2 <- tab %>%
  ...
  geom_label_repel(aes(x = Type, label = Freq),
            colour = "#000000", position=position_stack(reverse = TRUE, vjust = 0.7), size = 4.5) + 
...
#pdf("../submission/Figure2.pdf", width=10, height=3.5, compress=F)

### Export PNG or JPEG 600 dpi: 6000 x 2100px
#png("../submission/Figure2.png", width = 6000, height = 2100, res = 600)
jpeg("../submission/Figure2.jpg", width = 6000, height = 2100, res = 600, quality = 100)
print(fig2)
dev.off()
