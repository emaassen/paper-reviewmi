...
nrow(df0);nrow(df1)                     # we attempt to test MI for 141 + 153 comparisons
count_articles(df0);count_articles(df1) # across 10 + 25 articles

# Comparisons for which we could access the data
df01 <- filter(df0, open_data == 1)       # filter out comparisons that had open data
df11 <- filter(df1, open_data == 1)       # filter out comparisons that had open data
nrow(df01);nrow(df11)                     # 73 + 152 comparisons
count_articles(df01);count_articles(df11) # across 7 + 25 articles


# Comparisons for which we could make a grouping variable
df011 <- filter(df0, open_group == 1)       # filter out comparisons that had open group
df111 <- filter(df1, open_group == 1)       # filter out comparisons that had open group
nrow(df011);nrow(df111)                     # 65 + 152 comparisons

# Comparisons for which we could make a scaling variable
df0111 <- filter(df0, open_scale == 1)       # filter out comparisons that had open scale
df1111 <- filter(df1, open_scale == 1)       # filter out comparisons that had open scale
nrow(df0111);nrow(df1111)                    # 10 + 148 comparisons
...
nrow(df0);nrow(df1)                                        # 9 + 148 comparisons

# Comparisons for which we could do a MI test
df0 <- filter(df0, mitest_step4 == 1)                      # filter out comparisons that could do a MI test
df1 <- filter(df1, mitest_step4 == 1)                      # filter out comparisons that could do a MI test
nrow(df0);nrow(df1)                                        # 9 + 148 comparisons
count_articles(df0);count_articles(df1)                    # across 3 + 23 articles

# Comparisons for which MI held
df0y <- filter(df0, miresult_step4 == 1)                    # filter out comparisons that found MI
df1y <- filter(df1, miresult_step4 == 1)                    # filter out comparisons that found MI
nrow(df0y);nrow(df1y)                                       # 0 + 72 comparisons

# Comparisons for which MI did not hold
df0n <- filter(df0, miresult_step4 != 1)                    # filter out comparisons that did not find MI
df1n <- filter(df1, miresult_step4 != 1)                    # filter out comparisons that did not find MI
nrow(df0n);nrow(df1n)                                       # 9 + 76 comparisons

# Levels of invariance 
table(df0$milevel_step4)                                    # nonconfig = 9
table(df1$milevel_step4)                                    # nonconfig = 76, config = 13, metric = 13, scalar = 46

count_articles(df0[df0$milevel_step4==0,]);nrow(df0[df0$milevel_step4==0,]) # 3 articles, 9 com
count_articles(df0[df0$milevel_step4==1,]);nrow(df0[df0$milevel_step4==1,]) # 0 articles, 0 com
count_articles(df0[df0$milevel_step4==2,]);nrow(df0[df0$milevel_step4==2,]) # 0 articles, 0 com
count_articles(df0[df0$milevel_step4==3,]);nrow(df0[df0$milevel_step4==3,]) # 0 articles, 0 com

count_articles(df1[df1$milevel_step4==0,]);nrow(df1[df1$milevel_step4==0,]) # 16 articles, 76 com
count_articles(df1[df1$milevel_step4==1,]);nrow(df1[df1$milevel_step4==1,]) # 6 articles, 13 com
count_articles(df1[df1$milevel_step4==2,]);nrow(df1[df1$milevel_step4==2,]) # 6 articles, 13 com
count_articles(df1[df1$milevel_step4==3,]);nrow(df1[df1$milevel_step4==3,]) # 14 articles, 46 com
